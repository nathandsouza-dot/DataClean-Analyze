...
# Data cleaning using shell utilities
# SQLite database creation and loading
# Streamlit dashboard generation


INPUT_FILE=${1:-fifa_eda_stats.csv}
CLEANED_FILE="fifa_cleaned.csv"
DB_FILE="fifa_stats.db"
TEMP_DIR="temp_fifa"

echo "╔════════════════════════════════════════════╗"
echo "║   FIFA Stats Analysis Pipeline Started    ║"
echo "╚════════════════════════════════════════════╝"
echo ""


echo "[1/3] Data Cleaning with Shell Utilities..."
echo "─────────────────────────────────────────────"

mkdir -p $TEMP_DIR

# Remove empty lines and  whitespace
echo "  ✓ Removing empty lines and trimming whitespace..."
sed '/^$/d' "$INPUT_FILE" | sed 's/[[:space:]]*$//' > "$TEMP_DIR/step1.csv"

#  Remove duplicate rows 
echo "  ✓ Removing duplicate entries..."
head -1 "$TEMP_DIR/step1.csv" > "$TEMP_DIR/header.csv"
tail -n +2 "$TEMP_DIR/step1.csv" | sort -t',' -k2,2 -u > "$TEMP_DIR/step2_data.csv"

#  Remove rows with missing critical fields (ID, Name, Overall)
echo "  ✓ Filtering incomplete records..."
awk -F',' 'NF >= 5 && $1 != "" && $2 != "" && $5 != ""' "$TEMP_DIR/step2_data.csv" > "$TEMP_DIR/step3.csv"

#  Clean currency symbols and convert values (only in specific columns)
echo "  ✓ Normalizing currency and numeric fields..."
awk -F',' 'BEGIN {OFS=","} 
{
    # Only process Value (col 8) and Wage (col 9) and Release Clause (col 56)
    # Keep Name (col 2) and other text fields unchanged
    gsub(/€/, "", $8); gsub(/M/, "*1000000", $8); gsub(/K/, "*1000", $8);
    gsub(/€/, "", $9); gsub(/M/, "*1000000", $9); gsub(/K/, "*1000", $9);
    gsub(/€/, "", $56); gsub(/M/, "*1000000", $56); gsub(/K/, "*1000", $56);
    print
}' "$TEMP_DIR/step3.csv" > "$TEMP_DIR/step4.csv"

#  Sort by Overall rating (descending)
echo "  ✓ Sorting players by overall rating..."
sort -t',' -k5 -nr "$TEMP_DIR/step4.csv" > "$TEMP_DIR/sorted.csv"

# Step 6: Combine header with cleaned data
cat "$TEMP_DIR/header.csv" "$TEMP_DIR/sorted.csv" > "$CLEANED_FILE"

# Generate statistics using shell utilities
echo ""
...
echo "     Cleaned rows: $CLEANED_COUNT"
echo "     Duplicates removed: $DUPLICATES"

# Extract unique positions
echo ""
echo "  🎯 Top 10 Positions by player count:"
tail -n +2 "$CLEANED_FILE" | cut -d',' -f26 | sort | uniq -c | sort -rn | head -10 | \
    awk '{printf "     %-15s %s players\n", $2, $1}'

# Extract top clubs
echo ""
echo "  🏆 Top 10 Clubs by player count:"
tail -n +2 "$CLEANED_FILE" | cut -d',' -f7 | sort | uniq -c | sort -rn | head -10 | \
    awk '{printf "     %-25s %s players\n", $2, $1}'

# Top nationalities
echo ""
echo "  🌍 Top 10 Nationalities:"
tail -n +2 "$CLEANED_FILE" | cut -d',' -f4 | sort | uniq -c | sort -rn | head -10 | \
    awk '{printf "     %-20s %s players\n", $2, $1}'

echo ""
echo "✅ Data cleaning complete! Saved to: $CLEANED_FILE"
echo ""


echo "[2/3] Loading Data into SQLite Database..."
echo "─────────────────────────────────────────────"
...
.mode csv
.import $CLEANED_FILE players_temp
DELETE FROM players_temp WHERE rowid = 1;
...
CREATE INDEX idx_nationality ON players(Nationality);
CREATE INDEX idx_age ON players(Age);

-- Create analysis views
CREATE VIEW top_players AS
SELECT ID, Name, Age, Nationality, Overall, Potential, Club, Position, Value
FROM players 
...
SELECT 
    ...
    MAX(Overall) as MaxRating,
    MIN(Overall) as MinRating,
...
CREATE VIEW nationality_stats AS
SELECT 
    Nationality,
    COUNT(*) as PlayerCount,
    ROUND(AVG(Overall), 2) as AvgRating,
    MAX(Overall) as TopPlayer
FROM players
...
CREATE VIEW skill_analysis AS
SELECT 
    Name,
    Position,
    Overall,
    (Crossing + Finishing + HeadingAccuracy + ShortPassing + Volleys) / 5 as AttackSkill,
    (Dribbling + Curve + FKAccuracy + LongPassing + BallControl) / 5 as TechSkill,
    (Acceleration + SprintSpeed + Agility + Reactions + Balance) / 5 as PhysicalSkill,
    (Marking + StandingTackle + SlidingTackle + Interceptions) / 4 as DefenseSkill
FROM players
WHERE Position != 'GK'
ORDER BY Overall DESC
LIMIT 100;
...
SELECT 'Database Statistics' as Metric, '' as Value;
SELECT '─────────────────────', '──────────────';
SELECT 'Total Players', COUNT(*) FROM players;
SELECT 'Unique Clubs', COUNT(DISTINCT Club) FROM players WHERE Club != '';
SELECT 'Unique Positions', COUNT(DISTINCT Position) FROM players WHERE Position != '';
SELECT 'Unique Nationalities', COUNT(DISTINCT Nationality) FROM players WHERE Nationality != '';
SELECT 'Average Overall Rating', ROUND(AVG(Overall), 2) FROM players;
SELECT 'Highest Rated Player', Name || ' (' || Overall || ')' FROM players ORDER BY Overall DESC LIMIT 1;
EOF

echo ""
echo "✅ Database created successfully: $DB_FILE"
echo "   Created tables: players"
echo "   Created views: top_players, position_stats, club_stats, nationality_stats, skill_analysis"
echo ""


echo "[3/3] Generating Streamlit Dashboard..."
echo "─────────────────────────────────────────────"

cat > fifa_dashboard.py << 'DASHBOARD_EOF'
...
echo "✅ Streamlit dashboard created: fifa_dashboard.py"
echo ""
...
echo "   ✓ $CLEANED_FILE (cleaned data)"
echo "   ✓ $DB_FILE (SQLite database)"
echo "   ✓ fifa_dashboard.py (Streamlit app)"
echo ""
echo "🚀 To launch the dashboard, run:"
echo "   streamlit run fifa_dashboard.py"
echo ""
echo "💡 Or open the database directly:"
